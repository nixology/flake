  ...
  module = { config, lib, ... }:
    let
      ...
      transpose = attrsOfAttrs:
        ...
        in
        lib.genAttrs innerKeys (innerKey:
          lib.genAttrs outerKeys (outerKey:
    ...
    in
    {
      options.mod =
        let
          ...
          components = with lib.types;
            ...
            in
            lazyAttrsOf (oneOf [ component components ]);

          component = with lib.types; addCheck (attrsOf deferredModule)
            (component:
              let
            ...
            {
              ${lib.concatMapStringsSep "\n  " (class: "${class} = <module>; # optional - only for ${class} modules") cfg.mod.classes}
            }

            Notes:
              - Make sure the attribute key is spelled correctly [${lib.concatMapStringsSep ", " (class: class) cfg.mod.classes}]
        ...
        in
        {
          ...
          classes = lib.mkOption {
            ...
            default = [ "flake" "nixos" "darwin" "homeManager" ];
          };
      ...
      config =
        let
          ...
          flattenedComponents = flattenComponents { tree = cfg.mod.component; isLeaf = isComponent; };
          ...
          flake.modules =
            let
              ...
              components = lib.mapAttrs
                (class: namedModules:
                  lib.mapAttrs
                    (name: module: wrapModules name class "component" [ module ])
                    namedModules
                )
              ...
              aggregates =
                let
                  ...
                  aggregates = transpose (lib.mapAttrs
                    (name: aggregate:
                      let
                        components = componentsFor aggregate;
                      in
                      lib.mapAttrs
                        (class: modules: wrapModules name class "aggregate" modules)
                        (lib.zipAttrs components)
                    )
                    cfg.mod.aggregate);
                in
                aggregates;
